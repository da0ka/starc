...
***************************************************/
// compress
function o1e_p(A,cb,bs,up,done,rate){
	...
	var a=1,b=256,c,o=0,p=A[0],z=A.length,B=(cb&=7)|!!up<<4|!!z<<5,N=0,L=0,R=-1>>>0,fx=1<<(cb+=8),O=[],C=[],D=[],F=[],P=[],S=new Uint8Array(256);
...
function o1d_p(A,done,rate){
	...
	var sync="function"!=typeof rate,a=1,b=256,c=A[0],o=5,p=0,z=A.length,cb=8+(c&7),up=c>>4&1,L,R=-1>>>0,O=[],C=[],D=[],F=[],P=[],S=new Uint8Array(256);
	done=done||function(O){return O};rate=rate||function(){};
	O.b=gbits;O.c=cbt;
	for(;--o;)L=(L<<8|A[a++])>>>0;
