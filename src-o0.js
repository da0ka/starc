...
***************************************************/
// compress
function o0e(A,cb,bs,up,done,rate){
	// encode bits(p=0.5)
	...
	var a=0,b,c,d,e,f,n,o=0,x,z=A.length,B=(cb&=7)|!!up<<4,N=0,L=0,R=-1>>>0,as=0,fx=1<<(cb+=8),O=[],C=new Uint32Array(257),D=C.slice(1),F=C.slice(1),S=new Uint8Array(256),P=S.slice(),I=[];
	O.b=pbits;O.c=cbt;bs&=-1>>>8;
	bs=!bs?1<<24:bs<16?1<<bs+9:bs<1024?bs<<10:bs;
	for(;a<z;){
		// decide block size
		b=a+bs;if(b>z)b=z;
		for(n=a;n<b&&++F[A[n++]]<fx;);

		// check previous model, collect symbols(+counter)
		for(f=e=b=0;b<256;P[b++]=!c)
			c=F[b],P[b]-!c||e++,c?C[as]=F[S[as++]=x=b]-1:D[f++]=b;
		// write symbol table
		if(as<2){pbits(2,0),pbits(8,b=S[as=0]),pbits(cb,~a+n),F[b]=0,a=n;continue} // p=1, so no need to encode
...
function o0d(A,done,rate){
	...
	var a=1,b=A[0],c,d,e,o=5,x,as=0,cb=8+(b&7),up=b>>4,L,R=-1>>>0,O=[],C=new Uint32Array(257),D=C.slice(),F=C.slice(1),S=new Uint8Array(256),P=S.slice();
	done=done||function(O){return O};rate=rate||function(){};
	O.b=gbits;O.c=cbt;
	for(;--o;)L=(L<<8|A[a++])>>>0;
	for(;;){
		b=gbits(2);
		// read symbol table
		if(!b){for(P[b=gbits(8)]=0,c=gbits(cb);O[o++]=b,c--;);continue}
		if(b<2)
			...
			if(!as)return done(O,A.length,o)
		}
		// read probability
		b=gbits(2);c=0;
		...
		for(;;){
			...
			if(!--D[c]){// symbol should be exclude
