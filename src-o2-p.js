...
function o2d_p(A,done,rate){
	...
	function _(){
		...
		for(rate(a,z);;){
			...
			if(!--D[e][b]){
				if(--F[e][256]){
					if(up)for(c=0;c<x;)C[e][c+1]=C[e][c]+(F[e][c]=D[e][c++]) // update model by counter
				}else up?F[e][b]=0:F[e].fill(0);
				if(!--n)return wait0(_)
			}
		}
	}return wait0(_)
